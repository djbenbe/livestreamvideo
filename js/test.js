$(document).ready(function() {
    ...
    function checkAndUpdateTextFile() {
        $.get('song_list.txt', function(data) {
            if (data !== previousContent) {
                ...
                if (data === 'Ben - Met de beste muziek') {
                    playCustomVideo('video1.mp4');
                } else {
    ...
    // ...existing code...
function searchYouTubeVideo(query) {
    const proxyUrl = `/youtube-search?part=snippet&type=video&videoEmbeddable=true&maxResults=1&q=${encodeURIComponent(query)}&key=JOUW_API_KEY`;
    ...
    fetch(proxyUrl)
        ...
        .then(data => {
            if (data.items && data.items.length > 0) {
                ...
                console.warn("Geen embedbare video gevonden.");
            }
        })
        .catch(err => {
